      ...
      <div className="space-y-3">
        {mounts.map((mount, i) => (
          <ProgressBar
            ...
            value={mount.used}
            max={mount.total}
            label={mount.path.length > 15 ? `${mount.path.substring(0, 12)}...` : mount.path}
            tooltip={`Path: ${mount.path}\nUsage: ${formatBytes(mount.used)} / ${formatBytes(mount.total)} (${Math.round((mount.used / mount.total) * 100)}%)`}
            showPercentage
          />
        ))}
      </div>
